import SecondList from "./secondList";

export default async function SecondSection() {
  const secContent = {
    heading: "DevOps & Cloud Engineering",
    subtitle:
      "Our DevOps solutions streamline development and operations through automation, continuous integration, and continuous deployment (CI/CD). We help businesses build reliable cloud infrastructures, optimize workflows, and accelerate product delivery while maintaining security and performance. With modern DevOps practices, we ensure faster releases, reduced downtime, and improved operational efficiency.",
  };

  return (
    <>
      <div className="w-full h-full md:py-12 py-6 md:px-0 px-4 flex flex-col justify-center items-center">
        <div className="md:w-2/3 w-full  md:h-[75vh] h-full  relative overflow-hidden flex bg-[url('/secondSection.png')] bg-cover bg-center bg-no-repeat  rounded-lg shadow-lg">
          <div className="w-full h-full p-10 flex flex-col gap-3 justify-end items-start bg-black/50 text-white">
            <h2 className="text-left text-wrap md:w-1/2 w-full text-2xl">
              {secContent.heading}
            </h2>
            <p className="text-sm text-white/80">{secContent.subtitle}</p>
          </div>
        </div>
        <SecondList />
      </div>
    </>
  );
}
